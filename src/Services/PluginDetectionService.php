...
class PluginDetectionService {
    ...
    private function scan_repository_for_plugin( array $repository ): array {
        ...
        try {
            ...
            if ( ! empty( $root_php_files ) ) {
                ...
                // Try monorepo detection: check common subdirectories like plugins/{repo-name}/
                $monorepo_result = $this->scan_monorepo_for_plugin( $repository );
                if ( $monorepo_result['is_plugin'] ) {
    ...
    /**
     * Scan monorepo subdirectories for WordPress plugin files.
     * Checks common monorepo patterns like plugins/{repo-name}/ or packages/{repo-name}/
     *
     * @since 1.0.41
     * @param array $repository Repository data.
     * @return array Detection result.
     */
    private function scan_monorepo_for_plugin( array $repository ): array {
        $result = [
            'is_plugin' => false,
            'plugin_file' => '',
            'plugin_data' => [],
            'scan_method' => 'monorepo_scan',
            'error' => null,
            'monorepo_path' => '',
        ];
        ...
        // Common monorepo subdirectory patterns for WordPress plugins
        $monorepo_paths = [
            // Pattern: plugins/{repo-name}/{repo-name}.php (WooCommerce style)
            'plugins/' . $repo_name . '/' . $repo_name . '.php',
            // Pattern: plugins/{repo-name}/plugin.php
            'plugins/' . $repo_name . '/plugin.php',
            // Pattern: packages/{repo-name}/{repo-name}.php
            'packages/' . $repo_name . '/' . $repo_name . '.php',
            // Pattern: src/{repo-name}.php (less common)
            'src/' . $repo_name . '.php',
            // Pattern: plugin/{repo-name}.php (singular)
            'plugin/' . $repo_name . '.php',
        ];

        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            error_log( sprintf( 'SBI Detect: Trying monorepo paths for %s', $repository['full_name'] ) );
        }

        foreach ( $monorepo_paths as $file_path ) {
            $plugin_data = $this->scan_file_for_plugin_headers( $repository, $file_path );
            ...
            if ( ! is_wp_error( $plugin_data ) && ! empty( $plugin_data ) ) {
                ...
                $result['plugin_file'] = $file_path;
                $result['plugin_data'] = $plugin_data;
                $result['monorepo_path'] = dirname( $file_path );
                
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( sprintf( 'SBI Detect: Found plugin in monorepo path %s for %s', $file_path, $repository['full_name'] ) );
                }
    ...
    private function get_file_content( array $repository, string $file_path ) {
        ...
        $known_branch = $repository['default_branch'] ?? '';

        if ( empty( $owner_repo ) ) {
            return new WP_Error( 'invalid_repository', __( 'Repository name is missing.', 'kiss-smart-batch-installer' ) );
        }

        // Build list of branches to try - prioritize known branch, then try common alternatives
        // This unifies detection logic for both API (has accurate default_branch) and web scraper (defaults to 'main')
        $branches_to_try = [];
        if ( ! empty( $known_branch ) ) {
            $branches_to_try[] = $known_branch;
        }
        // Common default branch names - try all if the known one fails
        $common_branches = [ 'main', 'trunk', 'master' ];
        foreach ( $common_branches as $branch ) {
            if ( ! in_array( $branch, $branches_to_try, true ) ) {
                $branches_to_try[] = $branch;
            }
        }

        // Check cache first (using a branch-agnostic key for successful fetches)
        $cache_key_base = 'sbi_file_' . sanitize_key( $owner_repo . '_' . $file_path );
        $cached_content = get_transient( $cache_key_base );
        ...
        foreach ( $branches_to_try as $branch ) {
            ...
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( "SBI: Trying branch '{$branch}' - {$url}" );
            }

            $response = wp_remote_get( $url, $args );

            if ( is_wp_error( $response ) ) {
                $last_error = new WP_Error( 'file_fetch_failed', sprintf( __( 'Failed to fetch file: %s', 'kiss-smart-batch-installer' ), $response->get_error_message() ) );
            ...
            if ( 200 === $response_code ) {
                ...
                // Cache successful file content for 1 hour (branch-agnostic key)
                set_transient( $cache_key_base, $file_content, HOUR_IN_SECONDS );

                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( "SBI: Found file on branch '{$branch}' - " . strlen( $file_content ) . " bytes" );
                }

                return $file_content;
            }

            // 404 means file not found on this branch - try next
            if ( 404 === $response_code ) {
                $last_error = new WP_Error( 'file_not_found', sprintf( __( 'File not found: %s', 'kiss-smart-batch-installer' ), $file_path ) );
                continue;
            }

            // Other errors (rate limit, server error) - stop trying
            $last_error = new WP_Error( 'file_fetch_error', sprintf( __( 'HTTP error %d for: %s', 'kiss-smart-batch-installer' ), $response_code, $file_path ) );
            break;
        }

        // Cache the error for a short time to avoid repeated failed requests
        set_transient( $cache_key_base . '_error', $last_error, 300 ); // 5 minutes

        return $last_error ?? new WP_Error( 'file_not_found', __( 'File not found on any branch.', 'kiss-smart-batch-installer' ) );
    }
